...
int proto_proc(proto_struct *pProto, uint8_t *buf_in, uint16_t usLenIn)
{
    ...
    else
    {
        if (pProto->fRecvFirst || pProto->ucRecvTag != tag)
        {
            ...
            int res = PROTO_PROC_CMD(cmd, GET_PAYLOAD_PTR(buf_in), usLenIn-PROTO_SERVICE_BYTES_LEN, GET_PAYLOAD_PTR(pProto->bufPktResp), &usLenOut);
            if (res == 0)
            {
                ...
                PUT_CRC32(pProto->bufPktResp, PKT_HEADER_SIZE + usLenOut, CRC32(pProto->bufPktResp, PKT_HEADER_SIZE + usLenOut));
                if (PROTO_SEND(pProto->bufPktResp, PROTO_SERVICE_BYTES_LEN + usLenOut) != 0)
                {
                    return PROTO_ERR_IO;
                }
            }
